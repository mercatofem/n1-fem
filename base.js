...
import { revisar, motivoDeRevision, pendienteDeMano, porConfirmarPorGemini } from './revision.js';
...
const estado = (mov) => {
  ...
  // Y la liga cuando se va FUERA de las nuestras —«University of Findlay
  // (NCAA)»—, como ya lo dice la cronologia (24-09). Si es un equipo nuestro
  // su nombre ya lo dice todo.
  const ligaFuera = !equipoDestino && mov.ligaDestino ? ` (${mov.ligaDestino})` : '';
  const pistas = [
    adonde ? `Se va a ${adonde}${ligaFuera}` : null,
...
const frase = (m) => {
  ...
  const porQueRevisar = m.dudaDeteccion
    ?? (!otro && !deCasa && m.origenDudoso
      ...
      : null);
  // Y el «(*)» de lo que decidieron solo las regex con el modelo caido. Va con
  // la misma variable porque va en el mismo sitio de la fila —dentro del lado
  // derecho, que la celda es una rejilla—. Ver `porConfirmarPorGemini`.
  const revisar = (porQueRevisar
    ? `<span class="pill p-revisar" title="${esc(porQueRevisar)}">revisar</span>`
    : '')
    + (porConfirmarPorGemini(m)
      ? '<span class="pill p-confirmar" title="Movimiento por confirmar por Gemini: lo han leído las reglas de texto porque el modelo no respondió a tiempo">(*)</span>'
      : '');
...
const fotoAnuncio = (m, faldon = '') => {
  ...
  // Y si ESTE anuncio no trae nada, el cartel de su alta en el mismo club
  // (`cartelDeSuAlta`, 25-09): la baja de quien ficho y se fue —Alexia Salvado,
  // un comunicado en X sin imagen— salia con la silueta al lado de su cartel.
  const src = local || (m.imagen ?? '') || (m.cartelDeSuAlta ? `./datos/anuncios/${m.cartelDeSuAlta}` : '');
...
// La llamada del «(*)», visible: el `title` de la pastilla no existe en movil.
const NOTA_POR_CONFIRMAR = `<p class="nota-equipo">
  <b>(*)</b> Movimiento por confirmar por Gemini: lo han leído las reglas de texto porque el modelo no respondió a tiempo. Se confirma solo en cuanto lo lea.</p>`;
...
${filas.some(porConfirmarPorGemini) ? NOTA_POR_CONFIRMAR : ''}
